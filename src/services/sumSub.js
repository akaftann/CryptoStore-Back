import axios from 'axios'
import crypto from 'crypto'
//const fs = require('fs');
import FormData from 'form-data'
import dotenv from 'dotenv'
import { refreshSumsubToken, getByExternalId, createSumsubExternalId } from './users.js'
dotenv.config()

// These parameters should be used for all requests
const SUMSUB_APP_TOKEN = process.env.SUMSUB_TOKEN
const SUMSUB_SECRET_KEY = process.env.SUMSUB_SECRET_KEY
const SUMSUB_BASE_URL = 'https://api.sumsub.com'; 

let config = {};
config.baseURL= SUMSUB_BASE_URL;


// Make sure to specify 'Content-Type' header with value of 'application/json' if you're not sending a body for most of requests

// This function creates signature for the request as described here: https://developers.sumsub.com/api-reference/#app-tokens

const createSignature = (config) => {
  console.log('Creating a signature for the request...');

  let ts = Math.floor(Date.now() / 1000);
  const signature = crypto.createHmac('sha256',  SUMSUB_SECRET_KEY);
  signature.update(ts + config.method.toUpperCase() + config.url);

  if (config.data instanceof FormData) {
    signature.update(config.data.getBuffer());
  } else if (config.data) {
    signature.update(config.data);
  }

  config.headers['X-App-Access-Ts'] = ts;
  config.headers['X-App-Access-Sig'] = signature.digest('hex');

  return config;
}

// These functions configure requests for specified method

// https://developers.sumsub.com/api-reference/#creating-an-applicant
const createApplicant = (externalUserId, levelName) =>{
  console.log("Creating an applicant...");

  let method = 'post';
  let url = '/resources/applicants?levelName=' + encodeURIComponent(levelName);
  let ts = Math.floor(Date.now() / 1000);
  
  let body = {
      externalUserId: externalUserId
  };

  let headers = {
      'Accept': 'application/json',
      'Content-Type': 'application/json',
      'X-App-Token': SUMSUB_APP_TOKEN
  };

  config.method = method;
  config.url = url;
  config.headers = headers;
  config.data = JSON.stringify(body);

  return config;
}

// https://developers.sumsub.com/api-reference/#adding-an-id-document
/* function addDocument(applicantId) {
  console.log("Adding document to the applicant...");

  let method = 'post';
  let url = `/resources/applicants/${applicantId}/info/idDoc`;
  let filePath = 'resources/sumsub-logo.png';

  let metadata = {
      idDocType: 'PASSPORT',
      country: 'GBR'
  };

  let form = new FormData();
  form.append('metadata', JSON.stringify(metadata));
  
  let content = fs.readFileSync(filePath); 
  form.append('content', content, filePath);
  
  
  In case you'd like to upload images in base64 encoded string format:
  
  let content = 'data:image/png;base64,'
  let ext = content.substring("data:image/".length, content.indexOf(";base64"));
  let fileName = `image.${ext}`;
  let base64Data = content.split(',')[1];
  form.append('content', Buffer.from(base64Data, 'base64'), { fileName });
  

  let headers = {
    'Accept': 'application/json',
    'X-App-Token': SUMSUB_APP_TOKEN
  };

  config.method = method;
  config.url = url;
  config.headers = Object.assign(headers, form.getHeaders());
  config.data = form;

  return config;
} */

// https://developers.sumsub.com/api-reference/#getting-applicant-status-sdk
const getApplicantStatus = (applicantId) =>{
  console.log("Getting the applicant status...");

  let method = 'get';
  let url = `/resources/applicants/${applicantId}/status`;

  let headers = {
    'Accept': 'application/json',
    'X-App-Token': SUMSUB_APP_TOKEN
  };

  config.method = method;
  config.url = url;
  config.headers = headers;
  config.data = null;

  return config;
}

axios.interceptors.request.use(createSignature,  (error)=> {
    return Promise.reject(error);
  })

// https://developers.sumsub.com/api-reference/#access-tokens-for-sdks
const createAccessToken = (externalUserId, levelName = 'basic-kyc-level', ttlInSecs = 600) =>{
  console.log("Creating an access token for initializng SDK...");

  let method = 'post';
  let url = '/resources/accessTokens?userId=' + encodeURIComponent(externalUserId) + '&ttlInSecs=' + ttlInSecs + '&levelName=' + encodeURIComponent(levelName);

  let headers = {
      'Accept': 'application/json',
      'X-App-Token': SUMSUB_APP_TOKEN
  };

  config.method = method;
  config.url = url;
  config.headers = headers;
  config.data = null;

  return config;
}

// This section contains requests to server using configuration functions
// The description of the flow can be found here: https://developers.sumsub.com/api-flow/#api-integration-phases

        // Such actions are presented below:
        // 1) Creating an applicant
        // 2) Adding a document to the applicant
        // 3) Getting applicant status
        // 4) Getting access tokens for SDKs

export const generateAccessToken = async(externalUser, userId)=> {
  const externalUserId = externalUser + Math.random().toString(36).substr(2, 9);
  const levelName = 'basic-kyc-level';
  console.log("External UserID: ", externalUserId); 

  const applicant = await axios(createApplicant(externalUserId, levelName))
  console.log("Responce create applicant:\n", applicant.data);
  
  const applicantId = applicant.data.id;
  console.log("ApplicantID: ", applicantId);

  /* response = await axios(addDocument(applicantId))
  .then(function (response) {
    console.log("Response:\n", response.data);
    return response;
  })
  .catch(function (error) {
    console.log("Error:\n", error.response.data);
  }); */

  /* response = await axios(getApplicantStatus(applicantId))
  .then(function (response) {
    console.log("Response:\n", response.data);
    return response;
  })
  .catch(function (error) {
    console.log("Error:\n", error.response.data);
  }); */

  const accessToken = await axios(createAccessToken(externalUserId, levelName, 1200))
  await createSumsubExternalId(userId, externalUserId)
  await refreshSumsubToken(userId, accessToken.data.token)

  console.log("Response create access token:\n", accessToken.data);
  return accessToken.data
}

export const refreshAccessToken = async(encodedExtUserId)=> {
  const levelName = 'basic-kyc-level';
  console.log("External UserID: ", encodedExtUserId); 
  const user = await getByExternalId(encodedExtUserId)
  console.log('get by external_id..', user)
  const accessToken = await axios(createAccessToken(encodedExtUserId, levelName, 1200))
  await refreshSumsubToken(user.id, accessToken.data.token)

  console.log("Response create access token:\n", accessToken.data);
  return accessToken.data
}
